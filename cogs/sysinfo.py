import asyncio
import os, time, datetime, socket, discord
...
class SysInfo(commands.Cog):
    ...
    async def df(self, ctx):
        ...
        async with ctx.typing():
            ...
            msg = template.format(
                "Device", maxlen, "Total", "Used", "Free", "Used ", "Type", "Mount"
            )
    ...
    async def ifconfig(self, ctx):
        ...
        async with ctx.typing():
            ...
            try:
                ...
                await ctx.send(
                    "Unable to access network information due to PermissionError"
                )
                return
    ...
    async def nettop(self, ctx):
        ...
        async with ctx.typing():
            ...
            msg = "Total Bytes:           Sent: {0:<10}   Received: {1}\n".format(
                self._size(tot_after.bytes_sent), self._size(tot_after.bytes_recv)
            )
            ...
            for name in nic_names:
                ...
                msg += template.format(
                    ...
                    self._size(stats_after.bytes_sent - stats_before.bytes_sent) + "/s",
                )
                msg += template.format(
                    "bytes-recv",
                    self._size(stats_after.bytes_recv),
                    self._size(stats_after.bytes_recv - stats_before.bytes_recv) + "/s",
                )
                msg += template.format(
                    "pkts-sent",
        ...
        async with ctx.typing():
            ...
            for p in psutil.process_iter():
                try:
                    ...
                    vms = (
                        pinfo["memory_info"]
                        and int(pinfo["memory_info"].vms / 1024)
                        or "?"
                    )
                    rss = (
                        pinfo["memory_info"]
                        and int(pinfo["memory_info"].rss / 1024)
                        or "?"
                    )
                    memp = (
                        pinfo["memory_percent"]
                        and round(pinfo["memory_percent"], 1)
                        or "?"
                    )
    ...
    async def top(self, ctx, show_processes: bool = True):
        ...
        async with ctx.typing():
            ...
            if show_processes:
                template = (
                    "{0:<6} {1:<9} {2:>5} {3:>8} {4:>8} {5:>8} {6:>6} {7:>10}  {8:>2}\n"
                )
                msg = template.format(
                ...
                if wait:
                    ...
                    try:
                        msg = await ctx.bot.wait_for(
                            "message", check=same_author_check, timeout=10
                        )
...
async def infoo(self, ctx, args: str):
    async with ctx.typing():
        ...
        width = max(
            len("{:,}".format(int(n))) for n in [cpu_t.user, cpu_t.system, cpu_t.idle]
        )
        ...
        mem_ss = "Swap Memory" "\n\t{0:<6}: {1:>{width}}".format(
            ...
            "Used", self._size(mem_s.total - mem_s.free), mem_s.percent, width=width
        )
        ...
        width = max(
            len(self._size(n)) for n in [disk_u.total, disk_u.free, disk_u.used]
        )
